...
### 7.4 重複チェックと単一情報源（レビュー反映）

- **重複判定キー**: 同一 `set_id` かつ同一 `theme_id`（null 同士も一致とみなす）内で、`body` の完全一致（前後空白トリム後）を重複とみなす。重複は保存せずスキップし、取り込み欄で「既存」と表示する。
- **パースフォーマットの単一情報源**: 7.3 の `- [kind] 本文` フォーマットを唯一の正とし、「前提追加プロンプト」の生成テキスト（7.1）と取り込み欄のパーサ（6.3）、テスト（12章）はすべてこの定義を参照する。実装計画で定数化し、生成側とパース側が乖離しないようにする。

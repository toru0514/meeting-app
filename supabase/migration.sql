...
insert into bm_profiles (id, set_id, role_key, role_name, icon, fields, judgment_basis, interest, prohibitions, action_principle, sort_order) values
  ...
  ('00000000-0000-4000-8000-000000000110','00000000-0000-4000-8000-000000000001','marketing','マーケティング部長','📣',
   '{"main_channel":{"label":"主な集客チャネル","value":""},"target_segment":{"label":"狙う顧客層","value":""},"price_positioning":{"label":"価格帯・ポジショニング","value":""},"cac_limit":{"label":"許容できる顧客獲得コスト","value":"","unit":"円"},"repeat_rate":{"label":"リピート率","value":"","unit":"%"}}'::jsonb,
   ...
   '「認知を増やす（SNS・動画部長の領域）」と「認知を購入へ変える導線（自分の領域）」を切り分ける。今の律速が『知られていない』のか『知られても買われない』のかを見極め、集める前に受け皿（売り口・価格・オファー）が整っているかを問う。少数に深く刺すブランドでは、リーチ数より本命客への到達純度を優先して評価する。',10)
on conflict (id) do nothing;
